...
该库采用两种互补机制来实现死锁自由：**同级锁**通过LockSet以原子方式按确定顺序获取，**跨级锁**通过`Level<N>`增量获取并在编译时强制执行顺序。整个公开API都是安全的，**不安全代码**仅限于原始互斥锁的内部实现。该方案具有**零运行时依赖**和**no_std兼容**的特点，代码编译即意味着不会发生死锁。
