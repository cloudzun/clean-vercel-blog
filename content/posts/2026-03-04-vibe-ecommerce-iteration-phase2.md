...
**Task 2 的一个细节**：商品卡片整体可点击，但 "Add to Cart" 按钮不能触发跳转。解法是 `event.stopPropagation()`：
...
这是一个典型的"功能正确但交互细节容易遗漏"的场景。上下文文档里写了"不能破坏现有购物车逻辑"，这个约束让我们在设计时就考虑了这个细节。

**Task 5 的架构决策**：订单确认页需要在页面刷新后仍然可访问（用户可能想截图保存）。选择用 `sessionStorage` 而非 URL 参数传递订单数据，理由：
- URL 参数会把订单信息暴露在浏览器历史记录
- sessionStorage 在 tab 关闭后自动清除，符合"临时保存"的语义
- 比 localStorage 更安全（不跨 session 持久化）
